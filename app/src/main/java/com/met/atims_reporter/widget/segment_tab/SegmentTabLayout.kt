...
class SegmentTabLayout @JvmOverloads constructor(
    ...
    FrameLayout(context, attrs, defStyleAttr), AnimatorUpdateListener {
    ...
    private fun obtainAttributes(
        ...
        mIndicatorColor = ta.getColor(
            R.styleable.SegmentTabLayout_tl_indicator_color,
        ...
        mIndicatorCornerRadius =
            ta.getDimension(R.styleable.SegmentTabLayout_tl_indicator_corner_radius, -1f)
        indicatorMarginLeft = ta.getDimension(
            R.styleable.SegmentTabLayout_tl_indicator_margin_left,
            dp2px(0f).toFloat()
        )
        indicatorMarginTop =
            ta.getDimension(R.styleable.SegmentTabLayout_tl_indicator_margin_top, 0f)
        indicatorMarginRight = ta.getDimension(
            R.styleable.SegmentTabLayout_tl_indicator_margin_right,
            dp2px(0f).toFloat()
        )
        indicatorMarginBottom =
            ta.getDimension(R.styleable.SegmentTabLayout_tl_indicator_margin_bottom, 0f)
        isIndicatorAnimEnable =
            ta.getBoolean(R.styleable.SegmentTabLayout_tl_indicator_anim_enable, false)
        isIndicatorBounceEnable =
            ta.getBoolean(R.styleable.SegmentTabLayout_tl_indicator_bounce_enable, true)
        indicatorAnimDuration =
            ta.getInt(R.styleable.SegmentTabLayout_tl_indicator_anim_duration, -1).toLong()
        mDividerColor = ta.getColor(R.styleable.SegmentTabLayout_tl_divider_color, mIndicatorColor)
        mDividerWidth =
            ta.getDimension(R.styleable.SegmentTabLayout_tl_divider_width, dp2px(1f).toFloat())
        ...
        mTextSelectColor = ta.getColor(
            R.styleable.SegmentTabLayout_tl_textSelectColor,
            Color.parseColor("#ffffff")
        )
        mTextUnselectColor =
            ta.getColor(R.styleable.SegmentTabLayout_tl_textUnselectedColor, mIndicatorColor)
        mTextBold = ta.getInt(
        ...
        mTabPadding = ta.getDimension(
            ...
            if (mTabSpaceEqual || mTabWidth > 0) dp2px(0f).toFloat() else dp2px(10f).toFloat()
        )
        mBarColor = ta.getColor(
            R.styleable.SegmentTabLayout_tl_bar_color,
            Color.TRANSPARENT
        )
        mBarStrokeColor =
            ta.getColor(R.styleable.SegmentTabLayout_tl_bar_stroke_color, mIndicatorColor)
        mBarStrokeWidth =
            ta.getDimension(R.styleable.SegmentTabLayout_tl_bar_stroke_width, dp2px(1f).toFloat())
        ta.recycle()
    }
    ...
    private fun addTab(position: Int, tabView: View) {
        val tv_tab_title = tabView.findViewById<View>(
            R.id.tv_tab_title
        ) as TextView
        ...
        if (mTabWidth > 0) {
            lp_tab =
                LinearLayout.LayoutParams(mTabWidth.toInt(), LayoutParams.MATCH_PARENT)
        }
    ...
    private fun updateTabSelection(position: Int) {
        for (i in 0 until tabCount) {
            ...
            val tab_title = tabView.findViewById<View>(
                R.id.tv_tab_title
            ) as TextView
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    override fun onAnimationUpdate(animation: ValueAnimator) {
        val p =
            animation.animatedValue as IndicatorPoint
...
// show MsgTipView
    ...
    fun showMsg(position: Int, num: Int) {
        ...
        val tipView = tabView.findViewById<View>(
            R.id.rtv_msg_tip
        ) as MsgView
        if (tipView != null) {
    ...
    fun setMsgMargin(
        ...
        if (tipView != null) {
            ...
            lp.topMargin =
                if (mHeight > 0) (mHeight - textHeight).toInt() / 2 - dp2px(bottomPadding) else dp2px(
                    bottomPadding
                )
    ...
    internal inner class PointEvaluator :
        TypeEvaluator<IndicatorPoint> {
    ...
    init {
        ...
        mValueAnimator =
            ValueAnimator.ofObject(PointEvaluator(), mLastP, mCurrentP)
